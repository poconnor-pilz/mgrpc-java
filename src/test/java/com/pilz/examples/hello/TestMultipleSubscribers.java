...
public class TestMultipleSubscribers {

    private static Logger log = LoggerFactory.getLogger(TestMultipleSubscribers.class);
    ...
    public void testSubscription() throws InterruptedException {
        ...
        final String topic = Topics.out(DEVICE,"atesttopic");
        //Subscribe for responses
        CountDownLatch latch = new CountDownLatch(1);
        channel.subscribeForStream(topic, HelloReply.parser(), new StreamObserver<>() {
            @Override
            public void onNext(HelloReply helloReply) {
                log.debug(helloReply.getMessage());
            }
        ...
        //Now specify a replyTo
        request = RequestWithReplyTo.newBuilder()
                .setReplyToTopic(Topics.out(DEVICE,"atesttopic"))
                .setName("2").build();

        blockingStub.multipleSubscribers(request);
